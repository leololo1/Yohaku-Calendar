...
type FrameProps = {
  ...
  hideEmpty?: boolean;
  /** Crop a screenshot to a vertical slice expressed as fractions of its height. */
  cropTop?: number;
  cropHeight?: number;
  cornerRadius?: number;
};

// iPhone — bezelless screenshot with the display's rounded silhouette preserved.
export function Phone({
  ...
  const safeCropHeight = Math.max(0.05, Math.min(1, cropHeight));
  const safeCropTop = Math.max(0, Math.min(1 - safeCropHeight, cropTop));
  const isCropped = safeCropHeight < 1 || safeCropTop > 0;
  return (
    <div
      style={{
        position: "relative",
        aspectRatio: isCropped ? `1320 / ${2868 * safeCropHeight}` : "1320 / 2868",
        ...style,
        overflow: "hidden",
        ...(isCropped
          ? {
              borderTopLeftRadius: cornerRadius ?? 0,
              borderTopRightRadius: cornerRadius ?? 0,
              borderBottomRightRadius: 0,
              borderBottomLeftRadius: 0,
            }
          : { borderRadius: "12% / 5.5%" }),
        background: "#fff",
        boxShadow: "0 18px 54px rgba(52, 56, 58, 0.14)",
      }}
    >
      {resolved ? (
        <img
          ...
          style={{
            ...
            display: "block",
            left: 0,
            top: `${-(safeCropTop / safeCropHeight) * 100}%`,
            width: "100%",
            height: `${100 / safeCropHeight}%`,
            objectFit: "cover",
            objectPosition: "top",
          }}
          draggable={false}
