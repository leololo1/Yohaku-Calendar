...
function Caption({
  ...
  const yohakuHeadlineColors: Record<string, string> = {
    "yohaku-main": "#6B5350",
    "yohaku-themes": "#56645A",
    "yohaku-widget": "#4E6670",
  };
  const headlineFontFamily =
    slide.headlineFontFamily ||
    (theme.id === "yohaku"
      ? "HGMaruGothicMPRO, Meiryo, sans-serif"
      : undefined);
  const headlineColor =
    slide.headlineColor ||
    (theme.id === "yohaku" ? yohakuHeadlineColors[slide.id] : undefined) ||
    fg;
    ...
    <div style={{ textAlign: align, position: "relative", width: "100%" }}>
      ...
      <EditableText
        ...
        style={{
          ...
          fontWeight: headlineFontFamily ? 600 : 700,
          lineHeight: headlineFontFamily ? 1.08 : 0.96,
          letterSpacing: headlineFontFamily ? "0.025em" : -unit * 0.001,
          fontFeatureSettings: headlineFontFamily ? '"palt" 1' : undefined,
          color: headlineColor,
        }}
      />
    </div>
...
function backgroundFor(theme: Theme, inverted?: boolean) {
  return inverted ? theme.bgAlt : theme.bg;
}
...
function defaultElementZ(id: BuiltInElementId): number {
  if (id === "deviceSecondary" || id === "deviceTertiary" || id === "deviceQuaternary") return 2;
  ...
  return (
    <div
      style={{
        ...
        inset: 0,
        overflow: "hidden",
        background: slide.backgroundColor || backgroundFor(theme, inverted),
...
function SlideElements({
  ...
  const screenshotQuaternary = resolveScreenshot(slide.screenshotQuaternary, locale);
  ...
  const quaternaryRect = rectFor("deviceQuaternary", slide, defaults);
  ...
  function renderDevice(
    id: Exclude<BuiltInElementId, "caption">,
    rect: Rect,
    src: string,
    ...
    const widgetCrop =
      slide.id === "yohaku-widget"
        ? id === "device"
          ? { top: 0, height: 2539 / 2868 }
          : undefined
        : undefined;
    const elementAspect = widgetCrop
      ? cW / (cH * widgetCrop.height)
      : frameAspect;
    return (
      <Movable
        rect={toGlobal(rect)}
      ...
      >
        <Frame
          src={src}
          hideEmpty={hideEmpty}
          cropTop={widgetCrop?.top}
          cropHeight={widgetCrop?.height}
          cornerRadius={widgetCrop ? rect.width * 0.12 : undefined}
    ...
    <>
      {secondaryRect &&
        renderDevice(
          ...
          { opacity: screenshotTertiary || screenshotQuaternary ? 1 : 0.85 },
        )}
      {tertiaryRect && screenshotTertiary &&
        renderDevice("deviceTertiary", tertiaryRect, screenshotTertiary)}
      {quaternaryRect && screenshotQuaternary &&
        renderDevice("deviceQuaternary", quaternaryRect, screenshotQuaternary)}
